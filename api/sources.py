...
def load_meteorological_data_hourly(location, from_date, to_date):
    ...
    daily = list(
        zip(
            ...
            json_data["features"][0]["properties"]["parameters"]["GL"]["data"],
            json_data["features"][0]["properties"]["parameters"]["UU"]["data"],
            json_data["features"][0]["properties"]["parameters"]["VV"]["data"],
        )
    )
    daily_frame = DataFrame(
        [
            dict(
                timestamp=datetime.datetime.fromisoformat(day[0]),
                gl=day[1],
                uu=day[2],
                vv=day[3],
            )
...
if __name__ == "__main__":
    ...
    data = load_historical_energy_prices_hourly(start, end)

    data2 = load_meteorological_data_hourly((48.2157796, 16.3699576), start, end)
